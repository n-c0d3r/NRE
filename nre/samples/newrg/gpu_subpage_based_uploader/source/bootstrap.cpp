...
int main() {
	...
	auto application_p = TU<F_application>()(
		F_application_desc {
			.main_surface_desc = {
				.title = L"GPU SubPage-Based Uploader (NewRG)",
				.size = { 1024, 700 }
			}
		}
	);
	...
	TF_gpu_subpage_based_uploader<F_matrix4x4_f32> demo_gpu_subpage_based_uploader(
	...
	// application events
	{
		...
		{
			demo_gpu_subpage_based_uploader.reset();
	...
	// render_foundation event
	{
		NRE_NEWRG_RENDER_FOUNDATION_RG_REGISTER()
		{
			...
			{
				demo_gpu_subpage_based_uploader.RG_begin_register();

				demo_large_data_list[0] = T_identity<F_matrix4x4_f32>();
				demo_large_data_list[16] = T_identity<F_matrix4x4_f32>();
				demo_gpu_subpage_based_uploader.enqueue_upload(0);
				demo_gpu_subpage_based_uploader.enqueue_upload(0, 1);

				demo_gpu_subpage_based_uploader.RG_end_register();
			}
		};
