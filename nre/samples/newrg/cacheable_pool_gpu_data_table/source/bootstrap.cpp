...
int main() {
	...
	auto application_p = TU<F_application>()(
		F_application_desc {
			.main_surface_desc = {
				.title = L"Cacheable Pool GPU Data Table (NewRG)",
				.size = { 1024, 700 }
			}
		}
	);
	...
	TF_cacheable_pool_gpu_data_table<F_matrix4x4_f32> demo_gpu_data_table(
	...
	// render_foundation event
	{
		NRE_NEWRG_RENDER_FOUNDATION_RG_REGISTER()
		{
			...
			demo_gpu_data_table.RG_begin_register_upload();
			demo_gpu_data_table.T_enqueue_upload(
				id0,
				T_identity<F_matrix4x4_f32>()
			);
			demo_gpu_data_table.RG_end_register_upload();
		};
