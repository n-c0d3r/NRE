...
namespace nre {
	...
	TS<A_asset> F_nsl_shader_asset_factory::build_from_memory(const G_string& abs_path, const F_asset_buffer& buffer) {
		...
		auto shader_asset_p = TS<F_nsl_shader_asset>()(abs_path);
		...
		if(driver_index() == NRHI_DRIVER_INDEX_DIRECTX_11)
			output_language = E_nsl_output_language::HLSL;
#endif
		...
		shader_asset_p->load_from_compiled_result(
			compiled_result_opt.value()
		);
