...
namespace nre {
	...
	private:
		static TK<F_light_system> instance_ps;

	public:
		static NCPP_FORCE_INLINE TK<F_light_system> instance_p() {
	...
	private:
		TG_list<TK_valid<A_light>> light_p_list_;

		u8 last_channel_id_ = 0;
		TG_map<u64, u8> type_hash_code_to_channel_id_map_;

	public:
		NCPP_FORCE_INLINE const TG_list<TK_valid<A_light>>& light_p_list() const noexcept { return light_p_list_; }
	...
	public:
		NCPP_OBJECT(F_light_system);

	private:
		void _register(TKPA_valid<A_light> light_p);
		void deregister(TKPA_valid<A_light> light_p);
	...
	public:
		...
		NCPP_FORCE_INLINE F_light_mask mask() noexcept {

			return 0;
		}
		NCPP_FORCE_INLINE F_light_mask mask(auto type_hash_code) noexcept
		{
		...
		NCPP_FORCE_INLINE F_light_mask mask(auto type_hash_code_1, auto type_hash_code_2, auto... reset_type_hash_codes) noexcept {
		...
		NCPP_FORCE_INLINE F_light_mask T_mask() noexcept {
		...
		inline void for_each(auto&& functor) {
			...
			for(const auto& light_p : light_p_list_) {
		...
		inline void T_for_each(auto&& functor) {
			...
			if(search_mask)
			{
				for(auto it = light_p_list_.begin(); it != light_p_list_.end(); ++it) {

					if((*it)->mask() & search_mask)
						functor(*it);
				}
			}
			else for(auto it = light_p_list_.begin(); it != light_p_list_.end(); ++it) {
...
#define NRE_LIGHT_SYSTEM(...) (nre::F_light_system::instance_p())