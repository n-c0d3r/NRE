...
namespace nre
{
	...
	A_legacy_scene_render_view::A_legacy_scene_render_view(TKPA_valid<F_actor> actor_p, A_render_view_mask mask) :
		A_render_view(
			actor_p,
			mask
	...
	{
		NRE_ACTOR_COMPONENT_REGISTER(A_legacy_scene_render_view);
		NRE_ACTOR_COMPONENT_REGISTER(I_scene_render_view);

		actor_p->set_render_tick(true);
	}
	A_legacy_scene_render_view::~A_legacy_scene_render_view()
	{
	}

	void A_legacy_scene_render_view::setup_resources_internal() {
	...
	void A_legacy_scene_render_view::render_tick()
	{
		// update size
		if(main_rtv_p_)
		{
	...
	b8 A_legacy_scene_render_view::guarantee_resources() {
