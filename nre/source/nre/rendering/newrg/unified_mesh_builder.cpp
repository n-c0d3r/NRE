...
namespace nre::newrg
{
    ...
    )
    {
        ...
        //
        F_raw_clustered_mesh_data clustered_mesh_data;
        ...
        // build 0-level clusters
        {
            ...
            clustered_mesh_data.vertex_datas.reserve(meshlet_vertices.size());
            clustered_mesh_data.local_cluster_triangle_vertex_ids.reserve(meshlet_triangle_vertex_indices.size());
            ...
            clustered_mesh_data.cluster_headers.resize(meshlet_count);

            for(u32 meshlet_index = 0; meshlet_index < meshlet_count; ++meshlet_index)
            {
                auto& meshlet = meshlets[meshlet_index];
                auto& cluster_header = clustered_mesh_data.cluster_headers[meshlet_index];

                u32 vertex_count = meshlet.vertex_count;

                cluster_header.vertex_offset = clustered_mesh_data.vertex_datas.size();
                cluster_header.vertex_count = vertex_count;
                cluster_header.local_triangle_vertex_id_offset = clustered_mesh_data.local_cluster_triangle_vertex_ids.size();
                ...
                )
                {
                    ...
                    clustered_mesh_data.vertex_datas.push_back({
                ...
                for(u32 local_triangle_index = 0; local_triangle_index < meshlet.triangle_count; ++local_triangle_index)
                {
                    ...
                    clustered_mesh_data.local_cluster_triangle_vertex_ids.push_back(local_vertex_index_0);
                    clustered_mesh_data.local_cluster_triangle_vertex_ids.push_back(local_vertex_index_1);
                    clustered_mesh_data.local_cluster_triangle_vertex_ids.push_back(local_vertex_index_2);
                }
            }

            //
            clustered_mesh_data.cluster_group_graph.make_empty(meshlet_count);
            clustered_mesh_data.cluster_level_headers.push_back({
                .begin = 0,
        ...
        {
            F_cluster_id current_level_cluster_count = clustered_mesh_data.cluster_headers.size();
            F_cluster_id current_level_cluster_offset = 0;

            F_cluster_id max_cluster_count = current_level_cluster_count;

            F_global_vertex_id current_level_vertex_count = clustered_mesh_data.vertex_datas.size();
            F_global_vertex_id current_level_vertex_offset = 0;

            F_global_vertex_id current_level_local_cluster_triangle_vertex_id_count = clustered_mesh_data.local_cluster_triangle_vertex_ids.size();
            ...
            F_cluster_group_graph last_cluster_group_graph = clustered_mesh_data.cluster_group_graph;
            F_raw_clustered_geometry geometry = {
                .graph = clustered_mesh_data.cluster_headers,
                .shape = clustered_mesh_data.vertex_datas,
                .local_cluster_triangle_vertex_ids = clustered_mesh_data.local_cluster_triangle_vertex_ids
            };
            ...
            for(u32 cluster_level_index = 1; cluster_level_index < options.max_level_count; ++cluster_level_index)
            {
                ...
                F_raw_clustered_geometry groupped_geometry = H_clustered_geometry::build_next_level(
                    geometry,
                    cluster_group_graph
                );
                ...
                // store next level
                {
                    ...
                    F_cluster_id next_level_cluster_offset = clustered_mesh_data.cluster_headers.size();

                    F_global_vertex_id next_level_vertex_count = next_level_geometry.graph.size();
                    F_global_vertex_id next_level_vertex_offset = clustered_mesh_data.vertex_datas.size();
                    ...
                    F_global_vertex_id next_level_local_cluster_triangle_vertex_id_offset = clustered_mesh_data.local_cluster_triangle_vertex_ids.size();

                    clustered_mesh_data.cluster_headers.resize(next_level_cluster_offset + next_level_cluster_count);

                    clustered_mesh_data.vertex_datas.insert(
                        clustered_mesh_data.vertex_datas.end(),
                        next_level_geometry.shape.begin(),
                        next_level_geometry.shape.end()
                    );

                    clustered_mesh_data.local_cluster_triangle_vertex_ids.insert(
                        clustered_mesh_data.local_cluster_triangle_vertex_ids.end(),
                        next_level_geometry.local_cluster_triangle_vertex_ids.begin(),
                        next_level_geometry.local_cluster_triangle_vertex_ids.end()
                    );

                    //
                    clustered_mesh_data.cluster_group_graph.add_range(
                        cluster_group_graph,
                        next_level_cluster_offset
                    );
                    ...
                    //
                    {
                        ...
                        for(F_cluster_id i = 0; i < next_level_cluster_count; ++i)
                        {
                            ...
                            auto& next_level_cluster_header = clustered_mesh_data.cluster_headers[next_level_cluster_id];
                ...
                last_cluster_group_graph = cluster_group_graph;
        ...
        result.vertex_datas = clustered_mesh_data.vertex_datas;
        result.local_cluster_triangle_vertex_ids = clustered_mesh_data.local_cluster_triangle_vertex_ids;

        result.cluster_headers = clustered_mesh_data.cluster_headers;
        result.cluster_level_headers = clustered_mesh_data.cluster_level_headers;
