...
namespace nre {
	...
	public:
		...
		F_texture_2d_builder(u32 width, u32 height, const TG_span<u8>& data, E_format format) :
			width_(width),
			height_(height),
			texels_(width * height)
		{
		...
		F_texture_2d_builder(PA_vector2_u size, const TG_span<u8>& data, E_format format) :
			width_(size.x),
			height_(size.y),
			texels_(size.x * size.y)
		{
